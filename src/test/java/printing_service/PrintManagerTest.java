...
public class PrintManagerTest {
    ...
    @Test
    public void testSatisfiedPrintingPercentage_MoreCapacityThanPages() { // solution

        PrintManager pm = new PrintManager();
        int actual = pm.satisfiedPrintingPercentage(
                Arrays.asList(new Document(1, 1, 5)),
                Arrays.asList(new Printer(1, 5)));
    ...
    @Test
    public void testMissingDocuments_numPagesEqualsCapacity() { // solution line 71
        PrintManager pm = new PrintManager();
        List<Integer> actual = pm.missingDocuments(
                Arrays.asList(new Document(1, 1, 5)),
                Arrays.asList(new Printer(1, 5)));
    ...
    @Test
    public void testUnusedPrinters_withZeroPages() { // solution line 92

        PrintManager pm = new PrintManager();
        List<Printer> printers = Arrays.asList(new Printer(1, 5), new Printer(1, 2));
        List<Printer> actual = pm.unusedPrinters(
                Arrays.asList(new Document(1, 1, 5)),
                printers);

        Assertions.assertEquals(printers.subList(1, 1), actual);
    }
    ...
    public void testCanBePrinted(){
        ...
        Assertions.assertFalse(pm.canPrint(new Document(1, 1, 200), new Printer(1, 100)));
        Assertions.assertTrue(pm.canPrint(new Document(1, 1, 200), new Printer(1, 200)));
    ...
    public void testNextToBePrinted(){
        ...
        PrintManager pm = new PrintManager();
        Document doc = pm.nextToBePrinted(Arrays.asList(notChosen, chosen, notChosen));
        Assertions.assertEquals(chosen, doc);


        Assertions.assertNull(pm.nextToBePrinted(Arrays.asList()));
    }
